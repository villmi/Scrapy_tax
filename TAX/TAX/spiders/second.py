...
class QuerySpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        url1 = 'https://etax.jsgs.gov.cn/portal/queryapi/queryPageList.do?page=1&rows=10&orgid=%s&xzlb=2' % code
    ...
    def parse(self, response):
        ...
        f = open('/Users/vill/Desktop/TAX/TAX_cf/cf_%s.data' % code, 'a+')
        for item in items:
            # nsrmc = item["xk_nsrmc"]
            ...
            cf_xdr_mc = item["cf_xdr_mc"]
            nsrsbh = item['nsrsbh']
            cf_fr = item['cf_fr']
            cf_xdr_shxym = item["cf_xdr_shxym"]
            cf_wsh = item["cf_wsh"]
            cf_ajmc = item["cf_ajmc"]
            cf_sy = item["cf_sy"]
            cf_yj = item["cf_yj"]
            cf_jg = item["cf_jg"]
            cf_sxq = item["cf_sxq"]
            cf_xzjg = item["cf_xzjg"]
            f.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % (cf_xdr_mc, nsrsbh, cf_fr, cf_xdr_shxym, cf_wsh, cf_ajmc, cf_sy, cf_yj, cf_jg, cf_sxq, cf_xzjg ))
        print(len(city_code))
        ...
        if int(currentPage) <= int(totalPage):
            if int(currentPage) < int(totalPage):
                currentPage += 1
                url = 'https://etax.jsgs.gov.cn/portal/queryapi/queryPageList.do?page=%s&rows=10&orgid=%s&xzlb=2' % (
                currentPage, code)
            ...
            if int(currentPage) == int(totalPage):
                ...
                if count < len(location) and count < len(location):
                    ...
                    count = 0
                    code = location[int(count)]["xzqh_dm"]
                    url = 'https://etax.jsgs.gov.cn/portal/queryapi/queryPageList.do?page=1&rows=10&orgid=%s&xzlb=2' % code
                    yield scrapy.Request(url=url, callback=self.parse, meta={'count': count,
                                                                             'city': city,
                                                                             'city_index': int(city_index),
                                                                             'code': code})
